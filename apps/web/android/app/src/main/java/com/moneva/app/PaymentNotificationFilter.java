...
final class PaymentNotificationFilter {
    ...
    private static boolean looksFinancial(String title, String text, boolean fromPaymentApp) {
        ...
        /*
         * No verb, but a mark only a machine writes.
         *
         * This is what stops the vocabulary list being the whole defence. A
         * bank or wallet that phrases its alert in words nobody thought to add
         * - and there will always be one - still gets read, because it quoted
         * a reference or masked an account number, and those are not things
         * that appear in a message from a person.
         *
         * Deliberately BEFORE the marketing check: an advertisement does not
         * carry a transaction reference, so anything that has one is a receipt
         * whatever else is stapled to it.
         */
        if (hasMachineMark(lower)) return true;

        // No verb and no machine mark. Marketing language now decides it is an
        // advertisement quoting a figure rather than a receipt.
        if (MARKETING.matcher(lower).find()) return false;

        /*
         * A terse receipt from a payment app: an amount, no verb, no machine
         * mark, nothing selling anything. "Rs.161.70 to McDonald's" is the
         * shape Google Pay uses most, and requiring a verb threw it away - a
         * real payment on 11 September that had to be typed in by hand.
         *
         * Only for payment apps. An SMS app reaching this line is a message
         * with a number in it and no evidence whatsoever that money moved,
         * which is exactly the private text this filter exists to leave alone.
         */
     ...
     * MACHINE MARKS
     *
     * The vocabulary lists above are a losing game. Every bank writes its
     * alerts differently, every wallet invents its own phrasing, and each time
     * one of them says "spent" instead of "debited" a real payment is thrown
     * away until somebody notices and adds the word. The McDonald's miss was
     * that game being lost.
     *
     * These patterns play a different one. They look for the things a COMPUTER
     * puts in a payment message and a person never puts in a text to a friend:
     * a transaction reference, a masked account number, a running balance.
     * Nobody writes "UPI Ref 528417003921" to their mother.
     *
     * That is what makes them safe as well as strong. They raise what the app
     * catches without widening what it reads, because the thing they detect is
     * machine-generated by construction - so they can stand in for a verb even
     * on a messaging app, where the verb requirement exists to keep private
     * conversation out.
     * ------------------------------------------------------------------ */

    /**
     * A transaction reference. The strongest mark there is.
     *
     * UPI references are twelve digits, RRNs are twelve, bank txn ids vary -
     * so this keys on the LABEL and requires the value to contain a digit,
     * rather than trying to know every format. A label alone is not enough:
     * "reference number" appears in ordinary prose.
     */
    private static final Pattern TRANSACTION_REF = Pattern.compile(
            "\\b(?:upi|txn|transaction|order|payment|ref(?:erence)?|rrn|utr)\\s*"
                    + "(?:id|no|number|ref)?\\.?\\s*[:#-]?\\s*(?=[a-z0-9]*[0-9])[a-z0-9]{6,25}\\b",
            Pattern.CASE_INSENSITIVE);

    /**
     * A masked account or card number.
     *
     * "A/c XX1234", "card ending 4471", "**4471", "a/c no. xxxxxx8899". The
     * masking itself is the signal - a person writing about money does not
     * blank out the middle of a number.
     */
    private static final Pattern ACCOUNT_TAIL = Pattern.compile(
            "\\b(?:a/?c|acct|account|card)\\b[^0-9\\n]{0,18}?[x*\\u2022]{2,}\\s?[0-9]{3,6}\\b"
                    + "|\\b(?:a/?c|acct|account|card)\\b[^0-9\\n]{0,18}?(?:ending|ending\\s+in)\\s*[0-9]{3,6}\\b"
                    + "|[x*\\u2022]{4,}\\s?[0-9]{3,6}\\b",
            Pattern.CASE_INSENSITIVE);

    /**
     * A running balance. Only a statement quotes one.
     *
     * Bounded to the balance nouns rather than the bare word, because
     * "balance" alone turns up in ordinary sentences.
     */
    private static final Pattern BALANCE_LINE = Pattern.compile(
            "\\b(?:avl|avbl|available|closing|current|updated)\\s*(?:bal|balance)\\b"
                    + "|\\bbal(?:ance)?\\s*(?:is|:)\\s*(?:rs\\.?|inr|\\u20b9)?\\s?[0-9]",
            Pattern.CASE_INSENSITIVE);

    /**
     * Whether the text carries a mark only a machine would write.
     *
     * Any one of them is enough. Each is independently rare in conversation,
     * so requiring two would cost real payments - plenty of wallet alerts
     * carry a reference and nothing else - without buying much.
     */
    private static boolean hasMachineMark(String lower) {
        return TRANSACTION_REF.matcher(lower).find()
                || ACCOUNT_TAIL.matcher(lower).find()
                || BALANCE_LINE.matcher(lower).find();
    }
