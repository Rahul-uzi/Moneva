...
/** The drawings a tour step may name. TypeScript already rejects anything
    outside the union; this catches a name that is valid but was never drawn. */
const ILLUSTRATIONS: IllustrationName[] = [
  'wallet', 'ledger', 'chart', 'budget', 'goal', 'calendar', 'bell', 'search',
  'success', 'error', 'offline', 'welcome', 'add', 'secure', 'ask',
];

const VALID_ROUTES = new Set(['/', '/activity', '/plan', '/assistant']);
const VALID_SCENES = new Set<string>(ILLUSTRATIONS);
...
describe('guided tour visuals', () => {
  /* Colour is no longer a per-step decision - the whole set is drawn in one
     voltage pen - so what has to hold is that every step names a drawing the
     set actually contains. A typo used to render a blank scene silently. */
  it('draws only scenes the illustration set knows', () => {
